...
async function verify() {
    try {
        ...
        const query = `
            ...
            AND ea.academic_year = gtc.base_year -- STRICTLY CURRENT YEAR
            ORDER BY ea.course_code ASC
        `;

        const [results] = await pool.query(query, [userId]);
        console.log(`\nFound ${results.length} valid modules:`);
        results.forEach(r => {
            console.log(` - ${r.course_code} | EA Year: ${r.ea_year} | Cycle Year: ${r.cycle_year}`);
        });

        const hasMGTE = results.some(r => r.course_code === 'MGTE41252');
        const hasINTE = results.some(r => r.course_code.includes('21333'));

        console.log("\nMGTE41252 (2024/2025 assignment) found:", hasMGTE);
        console.log("INTE 21333 (2023/2024 assignment) found:", hasINTE);

        if (hasMGTE && !hasINTE) {
             console.log("\nVERIFICATION SUCCESS: Only current-year assignments are shown.");
        } else {
