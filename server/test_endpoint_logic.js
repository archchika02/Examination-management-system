    ...
    try {
        ...
        const query = `
            ...
            AND (
                ...
                OR (enrollments.academic_year IS NULL AND m.level IS NULL)
            )
        `;

        const [rows] = await pool.query(query, [baseYear, baseYear, baseYear, baseYear, baseYear, baseYear]);
        console.log("Students returned by the REAL endpoint logic:");
        console.table(rows);
